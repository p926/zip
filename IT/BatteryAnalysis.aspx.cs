...
public partial class IT_BatteryAnalysis : System.Web.UI.Page
{
    ...
    private void getDeviceCommunicationChartData(string serialNumber)
    {
        ...
        foreach (DataRow row in dtDeviceCommData.Rows)
        {
            ...
            object dtBatteryPercentage = row["BatteryPercentage"];
            object dtDoseRecord = row["DoseRecord"];
            object dtAdvertCount = row["AdvertCount"];
            object dtCPUTemp = row["CPUTemp"];
            
            if (dtBatteryPercentage != DBNull.Value && dtDoseRecord != DBNull.Value && dtAdvertCount != DBNull.Value && dtCPUTemp != DBNull.Value)
            {
                readDates.Add(dtReadDate.ToShortDateString());
                dataSeries1.Add(float.Parse(dtBatteryPercentage.ToString()));
                dataSeries2.Add(float.Parse(dtDoseRecord.ToString()));
                dataSeries3.Add(float.Parse(dtAdvertCount.ToString()));
                dataSeries4.Add(float.Parse(dtCPUTemp.ToString()));
            }
            else
            {
                continue;
