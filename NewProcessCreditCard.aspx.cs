...
// Modified On: 11/05/2014
// Modified By: thinh do
// Modified On: 11/25/2014
// This is only the 1st Draft for Review.
// Thinh Do will integrate the Chase/OrbitalAPI
// Also, Payments and PaymentTransactions tables will have to be properly designed before implementation.
// Some code has been either added as a place-holder, commented-out due to lack of knowledge of functionality or redundancy.
...
public partial class Finance_NewProcessCreditCard : System.Web.UI.Page
{    
    ...
    string defaultFinanceEmail = "jmack@mirion.com";
    ...
    protected void btnPrintSummaryStatement_Click(object sender, EventArgs e)
    {
        ...
        string mirionAddressByBransource = string.Format("2652 McGaw Ave{0}Irvine, CA 92614", Environment.NewLine);
            ...
            if (!retrieveFirstRowInfo)
            {
                ...
                if (this.instadoseProcessing)
                {
                    var actAddress = (from a in idc.Locations
                                      where a.AccountID == int.Parse(accountID) && a.IsDefaultLocation == true && a.Active == true
    ...
    private void PopulateActiveInvoicesGridView(string AccountID)
    {
        ...
        {
            // Query the AccountID.		
            var getActiveInvoices = (from a in idc.sp_AccountARHistory(Convert.ToInt32(AccountID))
                                     select new
                                     {
                                         AccountID = a.AccountID,
                                         CompanyName = a.CompanyName,
                                         InvoiceNo = a.InvoiceNo.ToString(),
                                         InvoiceDate = a.InvoiceDate.Value.ToShortDateString(),
                                         InvoiceAmt = a.InvoiceAmt,
                                         Balance = a.Balance,
                                         CurrencyCode = a.OrbitalCurrencyCode,
                                         ExponentCode = a.OrbitalExponentCode,
                                         a.OrderID,
                                         DaysSinceInvoice = (DateTime.Today-a.InvoiceDate.Value).Days
                                     }).ToList();

            // There are no records to display.
            // This is redundant.
            if (getActiveInvoices.Count == 0)
            {
            ...
            else
            {
                ...
                gvActiveInvoices.DataSource = getActiveInvoices;
                gvActiveInvoices.DataBind();
            }
            ...
            string companyName = (from a in idc.Accounts where a.AccountID == Convert.ToInt32(AccountID) select a.CompanyName).FirstOrDefault();
            this.lblCompanyName.Text = companyName.ToUpper();
        }
        else
        {
            // Query the AccountID.		            
            //DataTable getActiveInvoices = fr.GetCCInvoices(AccountID, null);
            var acctReq = new AccountRequests();
            var companyName = acctReq.GetCompanyName(AccountID).ToUpper();
            var getActiveInvoices = fr.GetARTransactions(AccountID)
                                        .Where(ar => ar.TypeCode == "S")
                                        .Select(ar => new
                                        {
                                            AccountID = ar.GDSAccount,
                                            CompanyName = companyName,
                                            InvoiceNo = ar.InvoiceNo.ToString().PadLeft(8,'0'),
                                            InvoiceDate = ar.InvoiceDate.Value.ToShortDateString(),
                                            InvoiceAmt = ar.Amount,
                                            Balance = ar.InvoiceBalance,
                                            CurrencyCode = (from c in idc.Currencies where c.CurrencyCode == ar.CurrencyDesc select c).FirstOrDefault().OrbitalCurrencyCode,
                                            ExponentCode = (from c in idc.Currencies where c.CurrencyCode == ar.CurrencyDesc select c).FirstOrDefault().OrbitalExponentCode,
                                            OrderID = new int?(),
                                            DaysSinceInvoice = (DateTime.Today - ar.InvoiceDate.Value).Days
                                        })
                                        .ToList();
            // There are no records to display.
            // This is redundant.
            //if (getActiveInvoices == null || getActiveInvoices.Rows.Count == 0)
            if (getActiveInvoices == null || getActiveInvoices.Count == 0)
            {
                this.totalBar.Visible = false;
                this.spnCreditCardInformation.Visible = false;
                this.btnPrintSummaryStatement.Enabled = false;
            ...
            else
            {
                ...
                gvActiveInvoices.DataSource = getActiveInvoices;//GetActiveInvoicesDataTableforGrid(getActiveInvoices); 
                gvActiveInvoices.DataBind();

                // Set Account ID to Hidden Field and Name (ID) to Label in the Modal/Dialog.
                this.hdnfldAccountID.Value = AccountID.ToString();
                this.lblAccountNumber.Text = AccountID.ToString();
                this.lblCompanyName.Text = companyName.ToUpper();//getCompanyName(getActiveInvoices).ToUpper();
            }                        
        }
