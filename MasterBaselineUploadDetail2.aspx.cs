/*
 * Maintain PO Receipts Instadose 2
 * 
 *  Created By: Tdo, 4/1/2013
...
public partial class TechOps_MasterBaselineUploadDetail2 : System.Web.UI.Page
{
    ...
    MasterBaselineUpload2 mbu;
    StreamReader masterFileContent ;
    StreamReader copyMasterFileContent;
    StreamReader tempCompFileContent;
    StreamReader sensCoefFileContent;
    StreamReader readerFileContent;
    StreamReader copyReaderFileContent;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {            
            ...
            else
            {
                ...
                var myItemNumberList = (from a in idc.POReceiptDetails
                                         where a.ReceiptID == Convert.ToInt32(ReceiptID)
                                         select a.ItemNumber).ToList();

                bool consistFDA20 = false;
                bool consistInstalink = false;

                if (myItemNumberList.Count() > 0)
                {
                    foreach (var itemNumber in myItemNumberList)
                    {
                        if (itemNumber.Trim() == "FDA20") consistFDA20 = true;
                        if (itemNumber.Trim() == "INSTALINK" || itemNumber.Trim() == "INSTALINKUSB") consistInstalink = true;
                    }
                }

                if (consistFDA20)
                {
                    MasterFileSection.Disabled = false;
                    TempCompFileSection.Disabled = false;
                    SensCoefFileSection.Disabled = false;
                }
                else
                {
                    MasterFileSection.Disabled = true;
                    TempCompFileSection.Disabled = true;
                    SensCoefFileSection.Disabled = true;
                }

                if (consistInstalink)
                {
                    ReaderFileSection.Disabled = false;
                }
                else
                {
                    ReaderFileSection.Disabled = true;
                }
            }           
        }
        catch 
    ...
    {
        bool uploadDevice = false;
        bool uploadReader = false;
        int countUploadDeviceFiles = 0;

        if (MasterFile.HasFile)
        {
            countUploadDeviceFiles++;
        }
        if (TempCompFile.HasFile)
        {
            countUploadDeviceFiles++;
        }
        if (SensCoefFile.HasFile)
        {
            countUploadDeviceFiles++;
        }

        if (!MasterFileSection.Disabled && countUploadDeviceFiles != 3)
        {
            VisibleErrors("The Master, Temperature Compensation, and Sensitivity Coefficient files are required for uploading Instadose 2.");
            return false;
        }

        if (!ReaderFileSection.Disabled && !ReaderFile.HasFile)
        {
            VisibleErrors("The Instalink file is required for uploading Instalink, InstalinkUSB.");
            return false;
        }

        //if (!MasterFile.HasFile && !TempCompFile.HasFile && !SensCoefFile.HasFile && !ReaderFile.HasFile)
        //{
        //    VisibleErrors("The Master, Temperature Compensation, and Sensitivity Coefficient files are required for uploading Instadose 2 or Instalink file is required for uploading Instalink, InstalinkUSB.");
        //    return false;
        //}
        
        //if (countUploadDeviceFiles != 0 && countUploadDeviceFiles != 3)
        //{
        //    VisibleErrors("The Master, Temperature Compensation, and Sensitivity Coefficient files are required for uploading Instadose 2.");
        //    return false;
        //}

        if (ReaderFile.HasFile)
        {
            uploadReader = true;
        }

        if (countUploadDeviceFiles == 3)
        {
            uploadDevice = true;
        }
        ...
        if (uploadDevice)
        {
            ...
            tempCompFileContent = new StreamReader(TempCompFile.FileContent);
            sensCoefFileContent = new StreamReader(SensCoefFile.FileContent);
        }

        if (uploadReader)
        {
            Stream fileContentStream = ReaderFile.FileContent;
            Stream copyFileContentStream = new MemoryStream();
            fileContentStream.CopyTo(copyFileContentStream);
            ...
            readerFileContent = new StreamReader(fileContentStream);
            copyReaderFileContent = new StreamReader(copyFileContentStream);            
        }


        mbu = new MasterBaselineUpload2(masterFileContent, tempCompFileContent, sensCoefFileContent, readerFileContent, uploadDevice,uploadReader, this.ReceiptID);
    ...
    private void WritePO_ReceiptDDFile(string pPathFile, Hashtable pLineIndexItemNumberPair)
    {
        ...
        System.IO.StreamReader instalinkFileStreamReader;
        ...
        try
        {
            ...
            using (CsvFileWriter writer = new CsvFileWriter(pPathFile))
            {
                    ...
                    if (masterFileStreamReader.Peek() != -1)
                    {
                        ...
                        while (masterFileStreamReader.Peek() != -1)
                        {
                            ...
                            if (myLine.Length > 0)
                            {
                                ...
                                myItemNumber = "FDA20";     // It is for ID2
                ...
                if (copyReaderFileContent != null)
                {
                    instalinkFileStreamReader = copyReaderFileContent;

                    //Read each line of the instalink file
                    if (instalinkFileStreamReader.Peek() != -1)
                    {
                        myLine = instalinkFileStreamReader.ReadLine(); //ignore the first line
                        while (instalinkFileStreamReader.Peek() != -1)
                        {
                            myLine = instalinkFileStreamReader.ReadLine();
                            if (myLine.Length > 0)
                            {
                                string[] columns = myLine.Split(',');

                                mySerialNo = columns[0];
                                myItemNumber = columns[3];     // It is either INSTALINK or INSTALINKUSB
                                myLineIndex = pLineIndexItemNumberPair.ContainsKey(myItemNumber) ? (string)pLineIndexItemNumberPair[myItemNumber] : "";
            ...
            //using (CsvFileWriter writer = new CsvFileWriter(pPathFile))
            //{
            //    //Insert each sn of the Master file into PO_ReceiptDD
            //    streamReader = copyMasterFileContent;
                
            //    if (streamReader.Peek() != -1)
            //    {
            //        myLine = streamReader.ReadLine(); //ignore the first line (column header line) then go through all the rows of Master files
            ...
            //    //Insert each sn of the uploaded devices, which are missing master info, into PO_ReceiptDD                
            //    DataTable dtValidationError = mbu.ValidationErrorMessage();
            ...
            //    foreach (DataRow error in dtValidationError.Rows)
            //    {
            //        bool uploadAnyway = (bool)error["Commit"];
    ...
    {        
        string url = "MasterBaselineUploadReport2.aspx?receiptID=" + pReceiptID + "&resultMessages=" + pResultMessages;
