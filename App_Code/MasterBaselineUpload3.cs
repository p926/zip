...
/// </summary>
public class MasterBaselineUpload3
{
    private const string NEWDEVICE_GROUP = "PRECALAPP";
    private const string NEWDEVICE_STATUS = "Unknown";
    private const int NEWDEVICE_PRODUCT = 24;
    private const string NEWDEVICE_SKU = "INSTA10-B";
    ...
    public MasterBaselineUpload3(StreamReader pMasterFileContent, string pReceiptID)
    {
        ...
        try
        {
            ...
            foreach (DataRow masterRow in masterParser.Data.Rows)
            {
                ...
                if (isMasterRowMissingData(masterRow))
                {
                    ...
                    InsertValidationErrorTable(false, masterRow["PCBA Serial"].ToString().Trim(), message, "MasterRowDataValidation");
                    pass = false;
                }
            }
        }
        ...
        try
        {
            ...
            foreach (DataRow masterRow in masterParser.Data.Rows)
            {
                ...
                else
                {
                    ...
                    InsertValidationErrorTable(false, masterRow["PCBA Serial"].ToString().Trim(), message, "MasterRowDuplicateValidation");
                    pass = false;
                }
            }
        }
    ...
    private bool MasterSerialNoFormatValidation()
    {
        ...
        try
        {
            ...
            foreach (DataRow masterRow in masterParser.Data.Rows)
            {
                ...
                if (Int32.Parse(serialNo) >= 6000000 || Int32.Parse(serialNo) < 5000000)
                {
                    string message = "Master file, S/N: " + serialNo + " is out of the format 5xxxxxx.";
    ...
    private bool MasterSerialNoExistValidation()
    {
        ...
        try
        {
            ...
            //    sqlCmd.Parameters["@ItemNumber"].Value = "PCBA";
            ...
            //            InsertValidationErrorTable(false, masterRow["PCBA Serial"].ToString().Trim(), message, "MasterSerialNoExistValidation");
            //            pass = false;
            //        }
            //    }
            //}
            ...
            foreach (DataRow masterRow in masterParser.Data.Rows)
            {
                ...
                if (exists > 0)
                {
                    message = "Master file, S/N: " + masterRow["PCBA Serial"].ToString().Trim() + " is already existed in DeviceInventory.";
                    InsertValidationErrorTable(false, masterRow["PCBA Serial"].ToString().Trim(), message, "MasterSerialNoExistValidation");
                    pass = false;
                }
            }
    ...
    public void Execute(ref string resultMessages)
    {
        try
        {
            ...
            string boxNumber = "100";        // Grab lot/box number.
            ...
            foreach (DataRow row in masterParser.Data.Rows)
            {
                ...
                string lf = row["LF"] == null ? "" : row["LF"].ToString().Trim();
                string iTSBase = row["ITS base/lid machine used (A/B)"] == null ? "" : row["ITS base/lid machine used (A/B)"].ToString().Trim();
                int? iTSBaseRow = row["ITS base/lid machine row to which the leadframe was placed during the sealing process"] == null ? (int?) null : int.Parse(row["ITS base/lid machine row to which the leadframe was placed during the sealing process"].ToString());
                string dieBatch = row["Die batch"] == null ? "" : row["Die batch"].ToString().Trim();
                string lFBatch = row["Leadframe batch"] == null ? "" : row["Leadframe batch"].ToString().Trim();
                string baseBatch = row["Base batch"] == null ? "" : row["Base batch"].ToString().Trim();
                string lidBatch = row["Lid batch"] == null ? "" : row["Lid batch"].ToString().Trim();
                double? preChargeVolt = (row["Pre-charge voltage used"] == null || row["Pre-charge voltage used"].ToString().Trim() == "") ? (double?) null : double.Parse(row["Pre-charge voltage used"].ToString());
                ...
                {
                    SerialNo = serialNo,
                    Product = (from p in IDC.Products where p.MaterialSKU == "PCBA" select p).FirstOrDefault(),
                    ...
                    LF = lf,
                    ITSBase = iTSBase,
                    ITSBaseRow = iTSBaseRow,
                    DieBatch = dieBatch,
                    LFBatch = lFBatch,
                    BaseBatch = baseBatch,
                    LidBatch = lidBatch,
                    PreChargeVolt = preChargeVolt,
