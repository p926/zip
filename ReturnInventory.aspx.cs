...
public partial class TechOps_ReturnInventory : System.Web.UI.Page
{
    ...
    string defaultID1ProductSKU = "INSTA10-B";
    string defaultID2EliteProductSKU = "INSTA20";
    string defaultIDPlusProductSKU = "INSTA PLUS";

    protected void Page_Load(object sender, EventArgs e)
    {
        Page.Title = "RMA - Technical Receipt of Badges";
    ...
    protected void btnFinish_Click(object sender, EventArgs e)
    {
        ...
        {
            this.VisibleErrorReview(errorMsg);
        ...
        this.VisibleMessages("Badges have been added and approved.");
    ...
    private AccountDevice getAccountDevice(string serialNumber)
    {
        // Create the object to return.
        ...
        try
        {
            ...
            addDev = (from ad in idc.AccountDevices
                      join di in idc.DeviceInventories on ad.DeviceID equals di.DeviceID
                      where di.SerialNo == serialNumber && ad.CurrentDeviceAssign == true
    ...
    private void LoadDeviceDataTable(List<string> serialNumberList)
    {
        ...
        dtDeviceDataTable.Columns.Add("Bucket", typeof(string));
        ...
        if (serialNumberList.Count > 0)
        {
            ...
            foreach (string serialNumber in serialNumberList)
            {
                ...
                if (serialNumber.Trim() != "")
                {
                    ...
                    //AccountDevice accountDevice = getAccountDevice(serialNumber.Trim());
                    ...
                    // This is when the device has not RECEIVED by shipping dept. If rd.Received == True, we know it was received by shipping
                    ...
                    returnDevice = (from rd in adc.rma_ReturnDevices
                                    ...
                                    orderby rinv.CreatedDate descending
                                    select rd).FirstOrDefault();
                    ...
                    if (returnDevice != null)
                    {
                        ...
                        string returnType = returnDevice.rma_Return.rma_ref_ReturnType.Type;
                        string returnReason = returnDevice.rma_Return.rma_ref_ReturnReason.Description.ToUpper();

                        // Create the data row to be added to the data table.
                        DataRow dr = dtDeviceDataTable.NewRow();

                        dr["SerialNo"] = serialNumber;
                        dr["AccountNo"] = returnDevice.rma_Return.AccountID;
                        dr["RequestNo"] = requestNo;
                        dr["RequestType"] = (returnType.ToUpper() == "RECALL") ? "Recall" : "Return";
                        ...
                        dr["Type"] = "Badge has been received for a return.";

                        int bucketNumber = 0;

                        if (returnType.ToUpper() == "RECALL")   // bucket 1 or bucket 5
                        {
                            if (returnReason == "BADGE REPLACE - MGR PERM NEEDED")
                                bucketNumber = 5;
                            else
                                bucketNumber = 1;
                        }
                        ...
                        {
                            if (int.Parse(deviceInventory.SerialNo) < 1000000)  // bucket 3
                                bucketNumber = 3;
                            else if (deviceInventory.Product.ProductSKU == defaultID1ProductSKU || deviceInventory.Product.ProductSKU == defaultID2EliteProductSKU || deviceInventory.Product.ProductSKU == defaultIDPlusProductSKU)   // bucket 4
                                bucketNumber = 4;
                            else   // bucket 2
                                bucketNumber = 2;
                        }
                        dr["Bucket"] = bucketNumber.ToString();
                    ...
                    //    errorList.Add(serialNumber, "Badge does not belong to any account.");
                    else if (initReturnDevice != null)    // Device has not received by Receiving Department.
                        errorList.Add(serialNumber, "Badge has not received by Shipping/Receiving Department.");
                    else if (returnInventory != null)    // It is showing device has already been RECEIVED and reviewed by Tech Ops.
                        errorList.Add(serialNumber, "Badge is already in Tech Ops' INV or Badge has not received by Shipping/Receiving Department.");
    ...
    // Bucket 1: Recalls
    // Bucket 2: Return Selmic/PCBA+Battery
    // Bucket 3: Return IMI/Partnertech
    // Bucket 4: Return PCBA
    protected void gvReview_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            switch (e.Row.Cells[11].Text.ToLower())
            {
                case "1":
                ...
                case "3":
                    ...
                    break;
                case "4":
                    e.Row.Cells[11].ForeColor = System.Drawing.Color.Navy;
                    break;
                case "5":
                    e.Row.Cells[11].ForeColor = System.Drawing.Color.Fuchsia;
                    break;
                default:
    ...
    private void CommitSendEmailNote()
    {
        ...
        bodyText += "<b>New Badges have been received and added to the return inventory by Tech.<br><br><br>Details listed below:</b>";
        ...
        if (rowCount > 0)
        {
            ...
            MailMessage msg2 = new System.Net.Mail.MailMessage("noreply@instadose.com", userEmail, "Badges received and added to Return Inventory by Tech.", bodyText);
    ...
    private void Commit()
    {
        ...
        for (int i = 0; i < this.gvReview.Rows.Count; i++)
        {
            ...
            string serialNo = gvRow.Cells[0].Text.Replace("\n", "").ToString().Trim();
            string returnType = gvRow.Cells[3].Text.Replace("\n", "").ToString().Trim();
            int accountID = int.Parse(gvRow.Cells[1].Text);
            int.TryParse(gvRow.Cells[2].Text, out returnRequestID);
            ...
            // Tdo, 9/02/2015
            // Change the Device Analysis Status. This is for Automation Inventory
            // Need to classify if this is a Recall Request or Return Request.
            // Idenity bucket for each device.
            // Bucket 1: Recalls
            // Bucket 2: Return Selmic/PCBA+Battery
            // Bucket 3: Return IMI/Partnertech
            // Bucket 4: Return PCBA
            ...
            string changedDeviceAnalysisStatus;

            if (returnType == "Recall")   // bucket 1 OR 5
            {
                rma_Return curReturn = (from r in adc.rma_Returns
                                        where r.ReturnID == returnRequestID && r.Active == true
                                        select r).FirstOrDefault();

                if (curReturn.rma_ref_ReturnReason.Description.ToUpper() == "BADGE REPLACE - MGR PERM NEEDED")  // bucket 5
                    changedDeviceAnalysisStatus = "Recalled Firmware";
                else  // bucket 1
                    changedDeviceAnalysisStatus = "Recalled-TechOps";
            }
            else
            {
                int sn = int.Parse(serialNo);
                if (sn < 1000000)  // bucket 3
                    changedDeviceAnalysisStatus = "Returned-Defective";
                else if (deviceInventory.Product.ProductSKU == defaultID1ProductSKU || deviceInventory.Product.ProductSKU == defaultID2EliteProductSKU || deviceInventory.Product.ProductSKU == defaultIDPlusProductSKU)   // bucket 4
                    changedDeviceAnalysisStatus = "Returned-TechOps";
                else   // bucket 2
                    changedDeviceAnalysisStatus = "Returned-Pending Process";
            }
            ...
            if (riTest != null) // not Null, status = already received by Mfg
            {
                ...
                try
                {
                    ...
                    if (returnRequestCount == inventoryCount)
                    {
                        ...
                        returnRequest.Completed = true;     // 1/22/2015
                ...
                try
                {
                    ...
                    // Set the device firstread is NULL when it comes to WH. 
                    // This makes sure the next wearer who receives the device when it goes out the WH again must go through initialize process.
                    ...
                    if (changedDeviceAnalysisStatus == "Returned-TechOps" || changedDeviceAnalysisStatus == "Returned-Pending Process")
                    {
                        isFail = false;
                    }
                    else
                    ...
                    myDeviceMnager.InsertDeviceInventoryAudit(deviceInventory.DeviceID, deviceInventory.DeviceAnalysisStatus.DeviceAnalysisStatusID, isFail, "TechOps Receive", "Portal");
                    ...
                    // Tdo, 6/13/2013. Update for ID2
                    // when Tech-ops received the returned devices, we set CurrentDeviceAssign flag = false. 
                    // This helps to identify the ID2 read comes from in house Tech-Ops or from client process. 
                    // The rule is if the device was never assigned to any device or CurrentDeviceAssign flag = false,
                    // We consider the ID2 read come from in house Tech-ops, otherwise come from client.
            ...
            else // --------- it seems the code in this else block is never stepped through. -------------------
            {
                ...
                try
                {
                    ...
                    // Tdo, 1/31/2014
                    // Change the Device Analysis Status and location. This is for Automation Inventory
                    // DeviceAnalysisStatus returnedStatus = (from s in idc.DeviceAnalysisStatus where s.DeviceAnalysisName == 'Recalled Returned Read' select s).FirstOrDefault();
                    DeviceAnalysisStatus returnedStatus = (from s in idc.DeviceAnalysisStatus where s.DeviceAnalysisName == changedDeviceAnalysisStatus select s).FirstOrDefault();
                    if (returnedStatus != null)
                    {
                        deviceInventory.DeviceAnalysisStatus = returnedStatus;
                ...
                if (serialNumber.Trim() != "")
                {
                    ...
                    if (RtnInvenList.Count > 0)
                    {
                        ...
                        foreach (var v in RtnInvenList)
                        {
                            ...
                            int accountNo = (v.rRet.AccountID != null) ? v.rRet.AccountID : 0;
                            string requestNo = (v.ri.ReturnID != null) ? v.ri.ReturnID.ToString() : "None";
                            ...
                            drE["RequestType"] = (requestType.ToUpper() == "RECALL") ? "Recall" : "Return";
